...
import { AlertCircle, Download, FileVideo, Maximize2, Pause, Play, RotateCcw, Volume2, VolumeX } from "lucide-react"
...
export function WebVideoPlayer({ src, title, className = "", poster = "/placeholder.jpg" }: WebVideoPlayerProps) {
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      ...
      // Try to detect video format
      const src = video.currentSrc || video.src
      if (src) {
        const extension = src.split('.').pop()?.toLowerCase()
        setVideoFormat(extension || 'unknown')
      }
    }
  ...
  const handleDownload = () => {
    ...
    link.href = src
    link.download = `${title}.${videoFormat || 'mov'}`
      ...
      <div className={`relative rounded-lg overflow-hidden bg-muted/20 aspect-video ${className}`}>
        <div className="w-full h-full flex flex-col items-center justify-center bg-muted/30 text-center p-4">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            صيغة الفيديو {videoFormat ? `(.${videoFormat})` : ''} غير مدعومة في المتصفح
          </p>
